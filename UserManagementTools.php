...
class UserManagementTools extends AbstractExternalModule
{
     ...
     * @return bool 
     */
    protected function pagePermitted($username=null, $page=null) {
    ...
    protected function getProjectAdmins() {
        ...
        foreach ($this->getProject()->getUsers() as $user) {
            if ($this->pagePermitted($user->getUsername())) $admins[] = $user;
        }
        return $admins;
    }
    ...
    public function redcap_user_rights($project_id) {
        $bad = array();
        foreach ($this->getProject()->getUsers() as $user) {
            if (!$this->pagePermitted($user->getUsername())) $bad[] = $user->getUsername();
        }
        if (count($bad) > 0) {
